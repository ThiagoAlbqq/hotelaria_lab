    ...
    if (buscar_cliente_por_cpf(c.cpf, &existente)) {
      ...
      if (opcao == 's' || opcao == 'S') {
        ...
        while (fgets(linha, sizeof(linha), original)) {
          sscanf(linha, "%d;%[^;];%[^;];%[^;];%[^;];%d", &cliente_temp.id,
                 cliente_temp.nome, cliente_temp.cpf, cliente_temp.telefone,
...
void relatorio_clientes() {
  ...
  while (fscanf(file, "%d;%[^;];%[^;];%[^;];%[^;];%d\n", &lista[count].id,
                lista[count].nome, lista[count].cpf, lista[count].telefone,
                lista[count].email, &lista[count].entradas) == 6) {
    count++;
  }
  fclose(file);
  ...
  for (int i = 0; i < count; i++) {
    printf("ID: %d | Nome: %s | CPF: %s | Entradas: %d\n", lista[i].id,
           lista[i].nome, lista[i].cpf, lista[i].entradas);
  }
}
